...
#2.1 Consultas tipo resumen:
#1. Consulta
select Categories.CategoryName as NombreCategoria, 
count(Products.ProductID) as NumeroProductos
from categories
inner join products
...
#2. Consulta
select Suppliers.SupplierName as ProductosProveedor,
count(Products.ProductID) as NumeroProductos
from Suppliers
inner join products
...
#3. Consulta
select Products.ProductName as Productos,
Products.Price as Ventas
from Products
...
#4. Consulta
select month(Orders.OrderDate) as Mes,
avg(OrderDetails.Quantity * Products.Price) as PromedioVentas
from Orders
inner join OrderDetails
...
count(Orders.OrderID) as Cantidad
from Customers
inner join Orders
...
order by Customers.CustomerID, Año;

#7. Consulta
select concat_ws(' ', Employees.FirstName, Employees.LastName) as empleado,
count(Orders.OrderID) as Cantidad
from Employees
inner join Orders
...
#8. Consulta
select Products.ProductName as Producto,
Categories.CategoryName as Categoria,
Products.Price as Precio
from Products
inner join Categories
on Products.CategoryID = Categories.CategoryID
where Products.Price = (select min(Price) 
from Products);
...
#12. Consulta
select concat_ws(" ", Employees.FirstName, Employees.LastName) as Empleado,
avg(OrderDetails.Quantity) as PromedioVentas
from Employees
inner join Orders
on Employees.EmployeeID = Orders.EmployeeID
inner join OrderDetails
...
#14. Consulta
select Categories.CategoryName as Categoria,
count(Products.ProductID) as Cantidad
from Categories
inner join Products
on Categories.CategoryID = Products.CategoryID
