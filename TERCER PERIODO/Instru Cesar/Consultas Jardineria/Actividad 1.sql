...
# 1.4.5 CONSULTAS MULTITABLA (COMPOCICIÓN INTERNA)
# 1 ejercicio 
# SQL 1
select cliente.nombre_cliente, empleado.nombre as NombreRepresentante, empleado.apellido1 as ApellidoRepresentante
...
# 2 ejercicio
# SQL 1
select cliente.nombre_cliente, empleado.nombre as NombreRepresentanteVenta, empleado.apellido1 as ApellidoRepresentanteVenta
...
# 8 ejercicio
# sql 1
select empleado.nombre, jefe.nombre as jefe
...
# 9 ejercicio
# sql 1
select empleado.nombre as empleado, jefe.nombre as jefe, jefe_jefe.nombre as jefe_del_jefe
from empleado, empleado as jefe, empleado as jefe_jefe
where empleado.codigo_jefe = jefe.codigo_empleado
and jefe.codigo_jefe = jefe_jefe.codigo_empleado;
...
inner join empleado as jefe
on empleado.codigo_jefe = jefe.codigo_empleado
inner join empleado as jefe_jefe
on jefe.codigo_jefe = jefe_jefe.codigo_empleado;
...
on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
where oficina.codigo_oficina is null
or cliente.codigo_cliente is null;
...
where producto.codigo_producto is null;
...
# 12 ejercicio
select empleado.*, jefe.nombre as jefe
from empleado
left join cliente
on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
...
# 9 ejercicio
select empleado.nombre, empleado.apellido1, count(cliente.codigo_cliente) as cantidad_clientes
from empleado
left join cliente
on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
where empleado.puesto = 'representante ventas'
group by empleado.codigo_empleado, empleado.nombre, empleado.apellido1;
...
sum(detalle_pedido.precio_unidad * detalle_pedido.cantidad) * 1.21 as total_con_impuestos
from producto
inner join detalle_pedido
on producto.codigo_producto = detalle_pedido.codigo_producto
...
# 1 ejercicio 
select cliente.nombre_cliente,
count(pedido.codigo_pedido) as cantidad_pedidos
from cliente
left join pedido
on cliente.codigo_cliente = pedido.codigo_cliente
group by cliente.codigo_cliente, cliente.nombre_cliente;
