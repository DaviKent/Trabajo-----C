DROP DATABASE IF EXISTS universidad_yalanda;
CREATE DATABASE universidad_yalanda CHARACTER SET utf8mb4;
USE universidad_yalanda;
...
#Hacer una consulta a la tabla asignatura 3
...
fecha_nacimiento, sexo, telefono, tipo
from persona
where tipo = 'Alumno'
and telefono is null;

#3.4
select concat_ws(' ', nombre, apellido1, apellido2) as Profesor, fecha_nacimiento,sexo,tipo
from persona
where tipo = 'profesor'
and year(fecha_nacimiento) = 1977
and sexo = 'M';

#3.5
select nombre, apellido1, apellido2, sexo, tipo
from persona
where apellido1 like '%z'
or apellido2 like '%z';

#3.1
select count(*) as Cau_yalanda
from persona
where tipo = 'alumno'; 

#3.2
select count(distinct id_alumno) as Cama_yalanda
...
#3.4
select nombre, apellido1, fecha_nacimiento, tipo
from persona
...
#3.5
select concat_ws(' ', persona.nombre, persona.apellido1) as profesor,
...
#3.7
select persona.nombre, persona.apellido1, persona.fecha_nacimiento,
asignatura.nombre as asgiantura
from persona
...
select concat_ws(' ', persona.nombre, persona.apellido1, persona.apellido2) as Alumno,
       concat_ws(' ', profesor_persona.nombre, profesor_persona.apellido1, profesor_persona.apellido2) AS profesor,
...
inner join persona as profesor_persona
on profesor.id_profesor = profesor_persona.id
where persona.tipo = 'alumno';

#3.9
select distinct asignatura.nombre as Asignatura,
...
where grado.nombre in ('Grado en Ingeniería Informática (Plan 2015)',
'Grado en Matemáticas (Plan 2010)'
);

#3.10
select distinct asignatura.nombre as Asignatura, asignatura.creditos,
asignatura.tipo,
...
where curso_escolar.anyo_inicio between 2014 and 2015;